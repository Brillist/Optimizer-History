...
public:
   ...
   BoundCt(
       int v,
       ConstrainedBound* srcBound,
       ConstrainedBound* dstBound);
   ...
   ConstrainedBound* srcBound() const
       { return _srcBound; }

   /** Get the destination bound. */
   ConstrainedBound* dstBound() const
       { return _dstBound; }
   //@}

   /** Apply pressure to lower bound. */
   void increment(utl::uint_t inc)
   {
      saveState();
      _v += inc;
      _dstBound->setLB(_v);
