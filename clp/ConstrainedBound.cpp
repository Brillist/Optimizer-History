...
ConstrainedBound::ConstrainedBound(
    Manager* mgr, bound_t boundType, int bound)
...
ConstrainedBound::propagateLB()
{
    ...
    uint_t inc = (_bound - _oldBound); ASSERTD (inc > 0);
...
ConstrainedBound::propagateUB()
{
    ...
    uint_t dec = (_oldBound - _bound); ASSERTD(dec > 0);
