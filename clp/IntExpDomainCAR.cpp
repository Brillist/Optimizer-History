...
IntExpDomainCAR::begin() const
{
    ...
    IntExpDomainCARit* it
        = new IntExpDomainCARit(this, uint_t_max, int_t_min);
    it->next();
    return it;
}
...
{
    IntExpDomainCARit* it
        = new IntExpDomainCARit(this, uint_t_max, int_t_max);
    return it;
}
...
IntExpDomainCAR::getPrev(int val) const
{
    ...
    uint_t idx
        = utl::binarySearch(
            ...
            utl::subtract<int>(),
            utl::find_ip);
    if (_values[idx] >= val) --idx;
    while (getCount(idx) == 0) --idx;
    return _values[idx];
}
