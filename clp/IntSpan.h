...
public:
    ...
    utl::uint_t getStateDepth() const
    { return _stateDepth; }

    void setStateDepth(utl::uint_t stateDepth)
    { _stateDepth = stateDepth; }

    void saveState(Manager* mgr)
    { if (_stateDepth < mgr->depth()) _saveState(mgr); }
    ...
    IntSpan* next(utl::uint_t idx) const
    { return _next[idx]; }

    IntSpan*& next(utl::uint_t idx)
    { return _next[idx]; }

    void setPrev(IntSpan* prev)
    { _prev = prev; }

    void setNext(utl::uint_t idx, IntSpan* span)
    { _next[idx] = span; }

    utl::uint_t getLevel() const
    { return _level; }
