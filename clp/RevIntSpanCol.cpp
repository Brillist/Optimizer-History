...
RevIntSpanCol::copy(const RevIntSpanCol& isc)
{
    ...
    for (const IntSpan* span = isc.head()->next();
         span != isc.tail();
         span = span->next())
    {
        // make a new span
...
RevIntSpanCol::backward(IntSpan** next)
{
    ...
    for (lvl = 0;
         (lvl <= maxLevel) && (span->next(lvl) == next[lvl]);
         ++lvl)
    {
        next[lvl] = span;
    }
...
RevIntSpanCol::eclipseNext(IntSpan** prev)
{
    ...
    next->prev() = prev[0];
    for (uint_t i = 0;
         (i < CLP_INTSPAN_MAXDEPTH) && (prev[i]->next(i) == span);
         ++i)
    {
