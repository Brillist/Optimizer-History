...
template <class T>
class RevSet : public ConstrainedVar
{
...
public:
    ...
    /** Intersection size. */
    utl::uint_t intersectCard(const RevSet<T>& rhs) const
    ...
    bool has(const utl::Object* key) const
    { return (_set.has(*key)); }

    /** Find the given object in the set. */
    iterator find(const utl::Object* key) const
...
RevSet<T>::backtrack()
{
    ...
    forEachIt(utl::Hashtable, removedItems, T, object)
        add(object);
    endForEach;

    // remove added items
    const utl::Hashtable& addedItems = delta->addedItems();
    forEachIt(utl::Hashtable, addedItems, T, object)
        remove(object);
    endForEach;
