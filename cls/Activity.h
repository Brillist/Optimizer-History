...
public:
    ...
    const clp::RangeVar& start() const
    { return _start; }
    ...
    clp::ConstrainedBound& esBound()
    { return _start.lowerBound(); }

    /** Get the latest-start bound. */
    clp::ConstrainedBound& lsBound()
    { return _start.upperBound(); }

    /** Get the earliest-finish bound. */
    clp::ConstrainedBound& efBound()
    { return _end.lowerBound(); }

    /** Get the latest-finish bound. */
    clp::ConstrainedBound& lfBound()
    { return _end.upperBound(); }

    /** Get the earliest-start bound. */
    const clp::ConstrainedBound& esBound() const
    { return _start.lowerBound(); }

    /** Get the latest-start bound. */
    const clp::ConstrainedBound& lsBound() const
    { return _start.upperBound(); }

    /** Get the earliest-finish bound. */
    const clp::ConstrainedBound& efBound() const
    { return _end.lowerBound(); }

    /** Get the latest-finish bound. */
    const clp::ConstrainedBound& lfBound() const
    { return _end.upperBound(); }
...
struct ActESordering : public std::binary_function<Activity*,Activity*,bool>
{
    bool operator()(const Activity* lhs, const Activity* rhs) const
    {
        if (lhs->es() != rhs->es()) return (lhs->es() < rhs->es());
