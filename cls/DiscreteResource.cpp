...
TimetableBoundOrderingDecCap::cmp(const Object* lhs, const Object* rhs) const
{
    ...
    if (lhsCap == rhsCap) return lhs->compare(*rhs);
...
void
DiscreteResource::addProvidedCapacity(
    int startTime,
    int endTime,
    utl::uint_t cap)
{
...
DiscreteResource::selectCapacity(utl::uint_t cap, utl::uint_t maxCap)
{
    ...
    forEachIt(utl::RBtree, _timetableBounds, TimetableBound, ttb)
...
DiscreteResource::doubleProvidedCap()
{
    IntSpan* span;
    const IntSpan* tail = _timetable.tail();
    for (span = _timetable.head()->next(); span != tail;
         span = span->next())
    {
        uint_t cap = span->v1();
