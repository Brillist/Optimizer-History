...
public:
   ...
   /** Get the prev ptr. */
   ResourceCalendarSpan* prev() const
      { return _prev; }

   /** Get the prev ptr. */
   ResourceCalendarSpan*& prev()
      { return _prev; }

   /** Get the next ptr. */
   ResourceCalendarSpan* next() const
      { return _next; }

   /** Get the next ptr. */
   ResourceCalendarSpan*& next()
      { return _next; }
private:
   void init(
      rcs_t type = rcs_undefined,
      rcs_status_t = rcss_undefined,
      utl::uint_t cap = 0);
...
struct ResourceCalendarSpanEndOrdering
   : public std::binary_function
            <ResourceCalendarSpan*,ResourceCalendarSpan*,bool>
{
   bool operator()(
      const ResourceCalendarSpan* lhs,
      const ResourceCalendarSpan* rhs) const
   {
