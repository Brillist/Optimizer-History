...
{
    virtual HashFunction* clone() const { return new CapPtHashCap(); }
...
{
    virtual HashFunction* clone() const { return new CapPtHashPt(); }
    virtual size_t hash(const Object* object, size_t size) const;
};
...
ResourceCapPts::copy(const Object& rhs)
{
    ...
    forEachIt(utl::RBtree, _capPts, CapPt, capPt)
        _capPtsHTcap += capPt;
        _capPtsHTpt += capPt;
    endForEach;
}
