...
Job::serialize(Stream& stream, uint_t io, uint_t)
{
    ...
    if (io == io_rd)
    {
        ...
        forEachIt(Array, array, JobOp, op)
            add(&op);
        endForEach
...
Job::initReleaseBound(Manager* mgr)
{
    ...
    // make _releaseBound
    _releaseBound
        = new ConstrainedBound(
            mgr,
            forward ? bound_lb : bound_ub,
            forward ? int_t_min : int_t_max);
    ...
    for (it = _allSops.begin(); it != lim; ++it)
    {
        ...
        CycleGroup* opCG
            = forward
            ? opAct->esBound().cycleGroup()
            : opAct->lfBound().cycleGroup();
...
//         str << Time(t).toString("$yyyy/$m/$d $h/$nn");
    ...
    str << ", ops:";
    for (jobop_set_id_t::const_iterator it = _ops.begin();
         it != _ops.end(); it++)
    {
        if (it != _ops.begin()) str << ",";
        str << (*it)->id();
    }
    str << ", sops:";
    for (jobop_set_id_t::const_iterator it = _allSops.begin();
         it != _allSops.end(); it++)
    {
        if (it != _allSops.begin()) str << ",";
        str << (*it)->id();
    }
    str <<'\0';
