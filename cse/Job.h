...
public:
    ...
    /** Get the preference. */
    utl::uint_t preference() const
    { return _preference; }

    /** Get the preference. */
    utl::uint_t& preference()
    { return _preference; }
    ...
    lut::uint_set_t workOrderIds() const
    { return _workOrderIds; }

    /** Get a set of workorder ids. */
    lut::uint_set_t& workOrderIds()
    { return _workOrderIds; }

    /** Get the root summary op. */
    SummaryOp* rootSummaryOp() const
    { return _rootSummaryOp; }

    /** Get the root summary op. */
    SummaryOp*& rootSummaryOp()
    { return _rootSummaryOp; }

    /// \name Schedulable Ops
    //@{
    /** Get list of all schedulable ops. */
    const jobop_set_id_t& allSops() const
    { return _allSops; }
...
/*     void schedule(JobOp* op, bool forward); */
    ...
    /** Get item quantity. */
    utl::uint_t itemQuantity() const
    { return _itemQuantity; }

    /** Get item quantity. */
    utl::uint_t& itemQuantity()
    { return _itemQuantity; }
    ...
    lut::period_t opportunityCostPeriod() const
    { return _opportunityCostPeriod; }

    /** Get the opportunity cost period. */
    lut::period_t& opportunityCostPeriod()
    { return _opportunityCostPeriod; }

    /** Get the lateness cost. */
    double latenessCost() const
    ...
    double latenessIncrement() const
    { return _latenessIncrement; }

    /** November 21, 2013 (Elisa) */
    /*  Get the lateness cost increment. */
    double& latenessIncrement()
    { return _latenessIncrement; }

    /** Get the lateness cost period. */
    lut::period_t latenessCostPeriod() const
    { return _latenessCostPeriod; }

    /** Get the lateness cost period. */
    lut::period_t& latenessCostPeriod()
    { return _latenessCostPeriod; }
    ...
    lut::period_t inventoryCostPeriod() const
    { return _inventoryCostPeriod; }

    /** Get the inventory cost period. */
    lut::period_t& inventoryCostPeriod()
    { return _inventoryCostPeriod; }

    /** Add an operation. */
    void add(const JobOp& op)
    { add((JobOp&)op); }
    ...
    /** Get cycle-group. */
    const clp::CycleGroup* cycleGroup() const
    { return _precedenceBound->cycleGroup(); }

    /** Get cycle-group. */
    clp::CycleGroup* cycleGroup()
    { return _precedenceBound->cycleGroup(); }

    /** Determine the relationship between self and the given job. */
    clp::cg_precedence_rel_t relationship(const Job* job) const
...
struct JobSuccessorDepthDecOrdering
   : public std::binary_function<Job*,Job*,bool>
{
   bool operator()(const Job* lhs, const Job* rhs) const
   {
