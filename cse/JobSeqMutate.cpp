...
bool
JobSeqMutate::execute(
    gop::Ind* ind,
    gop::IndBuilderContext* context,
    bool singleStep)
{
...
//     ASSERTD(string[_stringBase] != uint_t_max);
    ...
    if (swapJob == nullptr) swapJob = job; // basically it will do nothing
...
//     Job* swapJob = (*swapJobs)[idx];
    ...
    const cg_revset_t& allSuccCGs
        = firstJob->cycleGroup()->allSuccCGs();
    ...
    {
        if (nextJob == firstJob ||
            allSuccCGs.find(nextJob->cycleGroup()) != allSuccCGs.end())
        {
    ...
    for (job_vector_t::iterator it = changedJobs.begin();
         it != changedJobs.end(); it++)
    {
        Job* job1 = *it;
...
//                   << ", sid:" << op3->getSerialId() << std::endl;
    ...
    ////update string
    for (uint_vector_t::iterator it = _moveJobIdxs.begin();
         it != _moveJobIdxs.end(); it++)
    {
        uint_t idx = *it;
        Job* job1 = _jobs[idx];
...
JobSeqMutate::setJobs(const ClevorDataSet* dataSet)
{
    ...
    for (jobIt = jobs.begin(); jobIt != jobs.end(); jobIt++)
    {
        ...
        for (cgIt = cgCandidates.begin();
             cgIt != cgCandidates.end();
             ++cgIt)
        {
...
JobSeqMutate::selectActiveSwapJob(const job_vector_t& swapJobs)
{
    ...
    {
        Job* job = (*it);
        if (job->active()) jobs.push_back(job);
    }
