...
UTL_CLASS_IMPL(cse::JobSeqSelector, cse::Scheduler);
...
void
JobSeqSelector::initializeRandomInd(
   Ind* p_ind,
   const gop::DataSet* p_dataSet,
   RandNumGen* rng,
   void*)
{
   utl::cout << "ERROR(JobSeqSelector.cpp): in the incompleted code!"
             << utl::endl;
   ABORT();
}
...
void
JobSeqSelector::setSelectedJobSeq(
   ...
   SchedulingContext* context) const
{
   ...
   for (it = jobs.begin(); it != jobs.end(); ++it)
   {
      ...
      if ((active && job->serialId() != uint_t_max
           && sid == uint_t_max) ||
