...
{
    if (!rhs.isA(ResourceSequenceRule)) return Object::compare(rhs);
...
{
    if (!rhs.isA(ResourceSequenceList)) return Object::compare(rhs);

    const ResourceSequenceList& rsl = (const ResourceSequenceList&)rhs;
    int res;
    res = lut::compare(_id, rsl._id);
    if (res != 0) return res;
    res = compareContObjects(
        _rsl,
        rsl._rsl);
    return res;
}
...
ResourceSequenceList::findRule(
    uint_t lhsosid,
    uint_t rhsosid) const
{
    ...
    for (it = _rsl.begin(); it != _rsl.end(); ++it)
    {
        ...
        if ((lhsosid != uint_t_max)
            && (rsrLhsosid != uint_t_max)
            && (lhsosid != rsrLhsosid))
        {
            continue;
        }
        ...
        if ((rhsosid != uint_t_max)
            && (rsrRhsosid != uint_t_max)
            && (rhsosid != rsrRhsosid))
        {
            continue;
        }
...
void
ResourceSequenceList::add(
    uint_t lhsOpSequenceId,
    uint_t rhsOpSequenceId,
    uint_t delay,
    double cost)
{
    ResourceSequenceRule* rsr
        = new ResourceSequenceRule(
            lhsOpSequenceId,
            rhsOpSequenceId,
            delay,
            cost);
