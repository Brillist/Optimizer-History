...
UTL_CLASS_IMPL(mrp::ItemPlanRelation);
...
bool
PlanOrderingDecPref::operator()(const ItemPlanRelation* lhs, const ItemPlanRelation* rhs) const
{
...
ItemPlanRelation::ItemPlanRelation(ManufactureItem* item, ProcessPlan* plan, uint_t preference)
{
    init();
    _item = item;
    _plan = plan;
...
ItemPlanRelation::toString() const
{
    ...
    str << "ItemPlanRelation:"
        << ", item:" << _item->id() << ", plan:" << _plan->id() << ", preference:" << _preference
        << ", itemOwner:" << Bool(_itemOwner).toString()
        << ", planOwner:" << Bool(_planOwner).toString() << '\0';
