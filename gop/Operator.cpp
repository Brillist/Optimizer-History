...
Operator::toString() const
{
    ...
    str << name().c_str()
        ...
        << ", numValidVars:" << numVars()
        << ", sucRate:" << Float(p()).toString("precision:2")
        << ", successIter:" << _successIter
    ...
    bool* active)
{
    ...
    OperatorVar* var = new OperatorVar(
        idx, active, initSuccessIter, initTotalIter);
    _varSet.insert(var);
    if (var->p() > 0) _numValidVars++;
}
...
Operator::selectOperatorVarIdx()
{
    ...
    {
        OperatorVar* var = *it;
        if (!var->active()) continue;//deals with active
        if (successRate == 0) successRate = var->p();
        if (var->p() < successRate) break;
        vars.push_back(var);
    }
    ASSERTD(vars.size() > 0);
