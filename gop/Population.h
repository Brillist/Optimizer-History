...
public:
    ...
    friend class Ind;//??
    // typedefs
    typedef std::vector< StringInd<utl::uint_t>* >::iterator
        iterator;
    typedef std::vector<StringInd<utl::uint_t>* >::const_iterator
        const_iterator;
public:
    /** Constructor. */
    ...
    /** Set the size. */
    void setSize(utl::uint_t newSize)
    { _inds.resize(newSize); }

    /** Add all of the given population's individuals. */
    void add(
        const Population& rhs,
        utl::uint_t beginIdx = 0,
        utl::uint_t endIdx = utl::uint_t_max);

    /** Add an individual. */
    void add(const StringInd<utl::uint_t>& ind)
    { add((StringInd<utl::uint_t>&)ind); }

    /** Add an individual. */
    void add(StringInd<utl::uint_t>& ind)
    { if (isOwner()) add(ind.clone()); else { add(&ind); } }

    /** Add an individual. */
    void add(StringInd<utl::uint_t>* ind);

    /** Get the individual at the given index. */
    const StringInd<utl::uint_t>* get(utl::uint_t idx) const
    { ASSERTD(idx < size()); return _inds[idx]; }

    /** Get the individual at the given index. */
    StringInd<utl::uint_t>* get(utl::uint_t idx)
    { ASSERTD(idx < size()); return _inds[idx]; }

    /** Set the individual at the given index. */
    void set(utl::uint_t idx, const StringInd<utl::uint_t>& ind)
    { set(idx, (StringInd<utl::uint_t>&)ind); }

    /** Set the individual at the given index. */
    void set(utl::uint_t idx, StringInd<utl::uint_t>& ind)
    {
        if (isOwner()) set(idx, ind.clone());
        else { set(idx, &ind); }
    }
    ...
    void reserve(utl::uint_t numInds)
