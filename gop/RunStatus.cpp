...
RunStatus::update(bool complete, uint_t currentIter, uint_t bestIter, Score* bestScore)
{
    ...
    {
        std::cout << "RunStatus||currentIter " << currentIter << "||bestIter " << bestIter
                  << "||bestScore " << std::setiosflags(std::ios::fixed) << std::setprecision(0)
                  << bestScore->getValue() << "||resultType " << bestScore->getType() << std::endl;
    }
#endif
    _complete = complete;
