...
template <class T>
class StringInd : public gop::Ind
{
    UTL_CLASS_DECL_TPL(StringInd, T);
public:
    /** Constructor. */
    StringInd(utl::uint_t size)
    { init(); _string->setSize(size); }
    ...
    /** Get string pointer. */
    const String<T>* getString() const
    { return _string; }
    ...
    /** Set string pointer. */
    void setString(String<T>* string, bool owner = true)
    {
        if (_stringOwner) delete _string;
        _string = string;
        _stringOwner = owner;
    }

    /** Get the new String (const). */
    const String<T>* newString() const
    { return _newString; }
    ...
    */
    ...
    /** Get a gene. */
    const T& get(utl::uint_t idx)
    { return (*_string)[idx]; }

    /** Set a gene. */
    void set(utl::uint_t pos, utl::uint_t val)
    { (*_string)[pos] = val; }
...
{
    _string->crossover(
        (off1 == nullptr) ? nullptr : &off1->string(),
        (off2 == nullptr) ? nullptr : &off2->string(),
        rhs.string(),
        pos);
}
