...
static utl::uint32_t
challengeResponse(utl::uint32_t challenge)
{
...
connect(TCPsocket& server, InetHostname host, uint_t port)
{
    ...
    try
    {
        ...
        if (res == "000")
        {
            ...
            utl::uint32_t response = challengeResponse(challenge);
...
BT::run(int argc, char** argv)
{
    ...
    if ((cmd == "strip") && (numArgs >= 4))
    {
        ...
        for (;;)
        {
            ...
            bool filtered = false;
            for (auto regex_ : stripCommands)
            {
                auto& regex = utl::cast<Regex>(*regex_);
                if (command == regex)
                {
                    filtered = true;
                    break;
