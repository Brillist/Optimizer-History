...
void
MPSdataSet::initialize(Array& mpsItems, Array& mpsItemPeriods)
{
    ...
    forEachIt(Array, mpsItems, MPSitem, item) add(item);
    endForEach
...
MPSdataSet::resetMPSitemPeriods(Array& mpsItemPeriods)
{
    ...
    forEachIt(Array, mpsItemPeriods, MPSitemPeriod, period) add(period);
    endForEach

        for (mpsitem_set_id_t::const_iterator it = _mpsItems.begin(); it != _mpsItems.end(); it++)
    {
        MPSitem* item = *it;
        item->linkPeriods();
    }
...
MPSdataSet::resetWorkOrderIds(Array& workOrderIds)
{
    ...
    forEachIt(Array, workOrderIds, Uint, woId) _woIds.insert((uint_t)woId);
    endForEach
}
...
MPSdataSet::checkData() const
{
    ...
    for (mpsitem_set_id_t::const_iterator it = mpsItems().begin(); it != mpsItems().end(); it++)
