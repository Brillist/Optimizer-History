...
void
MPSrun::initialize(MPSdataSet* dataSet)
{
    delete _dataSet;
    _dataSet = dataSet;
}
...
MPSrun::run()
{
    ...
    for (mpsitem_set_id_t::iterator it = items.begin(); it != items.end(); it++)
    {
        ...
        utl::cout << utl::endl << "Calculate _pab and _mpsQuantity (calculate1)..." << utl::endlf;
#endif
        ...
        for (pIt = periods.begin(); pIt != periods.end(); pIt++)
            utl::cout << (*pIt)->toString() << utl::endlf;
        utl::cout << utl::endl << "Calculate _atp (calculate2)..." << utl::endlf;
#endif
...
MPSrun::generateWorkOrders()
{
    //     static uint_t id = 0;
    mpsitem_set_id_t items = _dataSet->mpsItems();
    for (mpsitem_set_id_t::const_iterator it = items.begin(); it != items.end(); it++)
    {
        time_t releaseTime = -1;
        mpsitemperiod_set_st_t periods = (*it)->periods();
        for (mpsitemperiod_set_st_t::const_iterator pIt = periods.begin(); pIt != periods.end();
             pIt++)
        {
            MPSitemPeriod* period = *pIt;
            // only generate workorders for unplanned wos.
            if (period->status() != periodstatus_unplanned)
                continue;
            ...
            if (quantity < eoq)
            {
                utl::cout << "WARNING: \"" << period->toString()
                          << "\" has a MPSquantity that is less than eoq(" << Uint(eoq).toString()
                          << ")" << utl::endlf;
            }
            ...
            {
                String* str = new String();
                *str = "EOQ of item " + Uint(period->item()->id()).toString() + "cannot be zero.";
                throw clp::FailEx(str);
            }
            while (quantity > 0)
            {
                ...
                WorkOrder* wo =
                    generateWorkOrder(_initId, period, (ManufactureItem*)item, quan, releaseTime);
...
MPSrun::generateWorkOrder(
    uint_t id, MPSitemPeriod* period, ManufactureItem* item, uint_t quantity, time_t releaseTime)
{
    ...
    std::string name = item->name() + "_" + (char*)(Uint(quantity).toString()) + "_" +
