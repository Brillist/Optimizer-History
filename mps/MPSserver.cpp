...
MPSserver::handle_initMPS(MPSclient* client, const Array& cmd)
{
    ...
    if ((cmd.size() != 3)
        || !cmd(1).isA(Array)
        || !allAre(cmd(1), CLASS(Item))

        || !cmd(2).isA(Array)
        || !allAre(cmd(2), CLASS(MPSitemPeriod)))
    {
...
void MPSserver::handle_getMPS(MPSclient* client, const Array& cmd)
{
#ifdef DEBUG_UNIT
    utl::cout << utl::endl
...
void MPSserver::handle_generateWorkOrders(MPSclient* client, const Array& cmd)
{
#ifdef DEBUG_UNIT
    utl::cout << utl::endl
    ...
    if ((cmd.size() != 4)
        || !cmd(1).isA(Array)
        || !allAre(cmd(1), CLASS(MPSitemPeriod))

        || !cmd(2).isA(Array)
        || !allAre(cmd(2), CLASS(Item))
    ...
    try
    {
...
#ifdef DEBUG_UNIT
        utl::cout << "   Input MPSItemPeriods: size:"
                  << Uint(dataSet->periods().size())
                  << utl::endlf;
        mpsitemperiod_set_itemid_t::iterator periodIt;
        for (periodIt = dataSet->periods().begin();
             periodIt != dataSet->periods().end();
             periodIt++)
        {
        ...
        utl::cout << "   Input existing woIds: size"
                  << Uint(dataSet->woIds().size())
                  << ", Ids:";
        uint_set_t::iterator idIt;
        for (idIt = dataSet->woIds().begin();
             idIt != dataSet->woIds().end();
                 idIt++)
        {
            utl::cout << (*idIt) << ", ";
        }
...
void MPSserver::handle_getWorkOrders(MPSclient* client, const Array& cmd)
{
#ifdef DEBUG_UNIT
    utl::cout << utl::endl
...
#ifdef DEBUG_UNIT
    utl::cout << "   Output wos: size:"
              << Uint(dataSet->workOrders().size())
              << utl::endlf;
    ...
    Bool(true).serializeOut(client->socket());
    lut::serialize<WorkOrder*>(dataSet->workOrders(),
                               client->socket(), io_wr);
