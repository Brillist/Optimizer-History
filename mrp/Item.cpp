...
uint_t
Item::checkAndConsumeInventory(inv_transaction_op_type_t opType,
                               uint_t opId,
                               time_t t,
                               uint_t reqQuantity,
                               inv_transaction_status_t status)
{
    ...
    if (t < 0)
        return reqQuantity;
    if (_inventoryRecord == nullptr)
        return reqQuantity;
    int_t availQuantity = _inventoryRecord->availableCapacity(t);
    if (availQuantity <= 0)
        return reqQuantity;
    ...
    if (diffQuantity <= 0)
    {
        ...
        InventoryTransaction* trans =
            new InventoryTransaction(t, status, -1 * (int_t)reqQuantity, opType, opId);
    ...
    else
    {
        ...
        InventoryTransaction* trans =
            new InventoryTransaction(t, status, -1 * (int_t)availQuantity, opType, opId);
        _inventoryRecord->add(trans);
        return diffQuantity;
    }
}
...
void
Item::consumeInventory(inv_transaction_op_type_t opType,
                       uint_t opId,
                       time_t t,
                       uint_t reqQuantity,
                       inv_transaction_status_t status)
...
void
Item::produceInventory(inv_transaction_op_type_t opType,
                       uint_t opId,
                       time_t t,
                       uint_t quantity,
                       inv_transaction_status_t status)
{
    ...
    InventoryTransaction* trans = new InventoryTransaction(t, status, quantity, opType, opId);
    _inventoryRecord->add(trans);
}
...
{
    MemStream str;
    str << "item:" << _id << ", name:" << _name.c_str() << ", MPSitem:" << _MPSitem;
    for (bom_vector_t::const_iterator it = _boms.begin(); it != _boms.end(); it++)
    {
        str << '\n' << "   " << (*it)->toString();
    }
