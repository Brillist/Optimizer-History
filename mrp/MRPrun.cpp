...
{
    if (_dataSetOwner) delete _dataSet;
    _dataSet = dataSet;
}
...
//     }
         ...
         woIt != wos.end(); woIt++)
    {
        ...
        job_set_pref_t jobs;
        createJobs(wo->id(),wo->name(),woQuantity,
                   wo->item(),wo->releaseTime(),jobs);

        wo->item()->consumeInventory(
            transoptype_workorder,
            wo->id(),
            wo->dueTime(),
            woQuantity,
...
#ifdef DEBUG_UNIT
            utl::cout << "wo_job:" << job->id()
                      << ", active:" << job->active()
                      << ", releaseT:" << getTimeString(job->releaseTime()).c_str()
                      << ", dueT:" << getTimeString(job->dueTime()).c_str()
                      << ", ivntC:"
                      << Float(job->inventoryCost()).toString("precisioin:2")
                      << ", opporC:"
                      << Float(job->opportunityCost()).toString("precision:2")
                      << ", lateC:"
                      << Float(job->latenessCost()).toString("precision:2")
                      << utl::endlf;
#endif
        }
    }
    ...
    job_set_pref_t& jobs)
{
    ...
    for (planIt = item->itemPlans().begin();
         planIt != item->itemPlans().end(); planIt++)
    {
    ...
    if (jobGroup != nullptr)
    {
        ...
        item->produceInventory(transoptype_jobgroup,jobGroup->id(),
                               releaseTime,jobQuantity,transstatus_planned);
    }
    else
    {
        item->produceInventory(transoptype_job,(*jobs.begin())->id(),
                               releaseTime,jobQuantity,transstatus_planned);
    }
    ...
    for (uint_t i = 0; i < item->boms().size(); i++)
    {
        ...
        if (jobGroup == nullptr)
        {
            ASSERTD(jobs.size() == 1);
            Job* job = *jobs.begin();
            childJobQuantity = cItem->checkAndConsumeInventory(
                transoptype_job,
                job->id(),
                releaseTime,
                jobQuantity * bom->quantity(),
                transstatus_planned);
        }
        else
        {
            childJobQuantity = cItem->checkAndConsumeInventory(
                transoptype_jobgroup,
                jobGroup->id(),
                releaseTime,
                jobQuantity * bom->quantity(),
                transstatus_planned);
        }
        if (childJobQuantity == 0) continue;
        ...
        if (dynamic_cast<PurchaseItem*>(cItem) != nullptr)
        {
            ...
            {
                Job* job = *jobs.begin();
                createPurchaseOrder(woId, cItem->name(), childJobQuantity,
                                    (PurchaseItem*)cItem, releaseTime,
                                    job->id(), requestertype_job);
            }
            else
            {
                createPurchaseOrder(woId, cItem->name(), childJobQuantity,
                                    (PurchaseItem*)cItem, releaseTime,
                                    jobGroup->id(), requestertype_jobgroup);
            }
        }
        else
        {
            createJobs(woId, cItem->name(),
                       childJobQuantity, (ManufactureItem*)cItem,
                       releaseTime, childjobs);
        }
        ...
        {
            Job* job = *jobIt;
            for (cjobIt = childjobs.begin();
                 cjobIt != childjobs.end(); cjobIt++)
            {
                Job* childJob = *cjobIt;
...
void MRPrun::createPurchaseOrder(
    uint_t woId,
    std::string poName,
    uint_t poQuantity,
    PurchaseItem* item,
    ...
    requester_type_t reqType)
{
    ...
    _initId++;
    while (_dataSet->purchaseOrderIds().find(_initId)
           != _dataSet->purchaseOrderIds().end())
    {
        _initId++;
    }
    po->id() = _initId;
    ...
    item->produceInventory(transoptype_purchaseorder, po->id(),
                           po->receiveDate(), po->quantity(),
                           transstatus_planned);
    _dataSet->add(po);
}
...
void MRPrun::createJobOps(Job* job, ProcessPlan* plan)
{
        ...
        for (stepIt = steps.begin(); stepIt != steps.end(); stepIt++)
        {
            ...
            if (stepOpIdMap.find(step->id()) != stepOpIdMap.end()) continue;
            _initId++;
...
//                 else utl::cout << "null";
                utl::cout << ", active:" << job->active()
                      ...
                      << ", dueT:" << getTimeString(job->dueTime()).c_str()
                      << ", item:" << job->itemId()
                      << ", qnty:" << job->itemQuantity()
                      << ", status:" << job->status()
                      << ", plan:" << plan->id()
                      << ", step:" << step->id()
                      << ", op:" << op->id()
                      << ", #resReqs:" << step->resourceRequirements().size()
                      << utl::endlf;
#endif
        ...
        for (stepIt = steps.begin(); stepIt != steps.end(); stepIt++)
        {
            ...
            {
                cse::PrecedenceCt* pct =
                    planstep->createPrecedenceCt();
...
void MRPrun::addResourceToClevorDS(Resource* res)
{
    ...
    else if (res->isA(CompositeResource))
    {
        if (_dataSet->findResource(res->id()) == nullptr)
        {
            ...
            cse::CompositeResource* cmpRes
                = ((CompositeResource*)res)->createCompositeResource();
...
void MRPrun::createResourceRequirements(ProcessStep* step, Job* job, JobOp* op)
{
    forEachIt(Array, step->resourceRequirements(), StepResourceRequirement, resReq)
        Resource* res = _dataSet->findMRPresource(resReq.resourceId());
        ASSERTD(res != nullptr);
        ...
        if (res->isA(DiscreteResource) || res->isA(CompositeResource))
        {
            ...
            forEachIt(Array, step->resourceCapPts(), StepAltResCapPts, resCapPts)
                if (res->id() == resCapPts.resourceId())
                {
                    capPts = &resCapPts;
                    break;
                }
            ...
            if (resReq.capacity() != uint_t_max &&
                step->processingTime() != uint_t_max &&
                capPts != nullptr &&
                capPts->numCapPts() > 0)
            {
        ...
        else if (res->isA(AlternateResources))
        {
            ...
            cse::ResourceGroupRequirement* cseResGrpReq = resReq.createResourceGroupRequirement(groupId);
            forEachIt(Array, step->resourceCapPts(), StepAltResCapPts, capPts)
                if (resIds.find(capPts.resourceId()) != resIds.end())
                {
                    if (resReq.capacity() != uint_t_max &&
                        step->processingTime() != uint_t_max &&
                        capPts.numCapPts() > 0)
            ...
            endForEach
            op->addResGroupReq(cseResGrpReq);
        }
    endForEach
}
