...
UTL_CLASS_IMPL(mrp::StepAltResCapPts, utl::Object);
...
StepAltResCapPts::copy(const Object& rhs)
{
    ...
    const StepAltResCapPts& rcps
        = (const StepAltResCapPts&)rhs;
...
void
StepAltResCapPts::addCapPt(uint_t cap, uint_t pt, rescappt_pt_per_t ptPer,
                           uint_t ptBatchSize, uint_t setupId)
{
    _caps.push_back(cap);
    _pts.push_back(pt);
...
StepAltResCapPts::createResourceCapPts(uint_t quantity) const
{
    ...
    for (uint_t i = 0; i < numCapPts; i++)
    {
        ...
        if (per == cappt_pt_per_batch)
        {
            ...
            {
                pt = pt *
                    (uint_t)ceil((double)quantity / (double)ptBatchSize);
            }
        }
